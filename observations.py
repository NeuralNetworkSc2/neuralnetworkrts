...
def extend_batch(batch, batch_size, prev_action, visible):
    mmr = np.zeros((6, ))
    mmr_val = min(max(int(3000 - 2500) // 1000, 0), 5)
    ...
    if visible:
        batch[0]['control_groups_hint'] = torch.zeros((batch_size, 1, 1))
    else:
        batch[0]['control_groups_hint'] = torch.zeros((batch_size, 1, 1))
