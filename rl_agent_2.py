...
from observations import (load_global_stats, extend_batch, to_cuda,  get_observation, screen_scalar,
                          minimap_categorical, minimap_scalar, Feat, screen_categorical, to_tensor)
...
class DefaultAgent(base_agent.BaseAgent):
    ...
    def step(self, obs: TimeStep):
        hidden = self.net.get_hidden(1)
        hidden = tuple([hid for hid in hidden])
        last_action = {
        ...
        print('-----------------------------------------------')
        inputs = get_observation(obs.observation)
        inputs = to_tensor(inputs)
        inputs = extend_batch((inputs, ), 1, last_action, True)[0]

        # inputs = to_cuda(inputs)
        outputs, next_hidden = self.net(inputs, hidden)
        hidden = tuple([hid for hid in next_hidden])
        ...
        return ac.FunctionCall(action, action_data)
...
class ActorCriticModel(nn.Module):
    ...
    def autoregressive_chain(self, lstm_output, gating,
                             # targets=None
                             ):
        outputs = {}
